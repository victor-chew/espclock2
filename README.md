...
In the end, it was decided to use the Timezone API found in more recent browsers to determine the user's timezone, then to call a simple PHP script running on a server whose clock is NTP-synchronized to return the timezone-adjusted time. This is deemed more effective than having to deal with not only the geolocation API policy changes, but also the translation of location to timezone which requires third party API (which normally requires signing up and registering for an API key).
